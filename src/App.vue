  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">视觉风格</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">空间类型</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">影视分景美术与搭景布光指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 影视戏剧分景与场景美术专家');
...
const metricsList = [
  {
    "key": "sceneArtisticAtmosphere",
    "label": "场景美术氛围感"
  },
  {
    "key": "cinematicLightingDepth",
    "label": "影视灯光质感深度"
  },
  {
    "key": "setDesignFeasibility",
    "label": "搭景搭建可行度"
  },
  {
    "key": "spatialLayoutRhythm",
    "label": "空间布局节奏感"
  },
  {
    "key": "narrativeImmersionMatch",
    "label": "叙事沉浸匹配分"
  }
];
const guideRules = [
  {
    "title": "灯光服务于叙事",
    "advice": "用冷暖光对比（如窗口冷月光与案头暖台灯光）表现人物内心的挣扎与孤独",
    "avoid": "切忌平光顺光照亮整个场景失去光影质感"
  },
  {
    "title": "道具真实陈设",
    "advice": "陈设道具要有生活留下的使用痕迹（如划痕、茶渍、磨损），避免过新过假",
    "avoid": "避免陈设与故事时代背景脱节的现代道具"
  },
  {
    "title": "层次感与空间纵深",
    "advice": "通过前景（如窗帘、植被）、中景（演员）、背景（灯火）拉开场景的垂直深度",
    "avoid": "严禁将人物紧贴白墙进行平淡无奇的拍摄"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[SHETING_SCORES\][\s\S]*?\[\/SHETING_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[SHETING_SCORES\](.*?)\[\/SHETING_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('sheting_history_records');
...
const saveHistory = () => {
  localStorage.setItem('sheting_history_records', JSON.stringify(historyList.value));
};
